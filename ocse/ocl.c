...
static void _handle_afu(struct ocl *ocl)
{
	...
	if (ocl->cmd != NULL) {
		...
		handle_buffer_write(ocl, ocl->cmd);          // just finishes up the read command structures
		...
		// handle_vc1_cmd(ocl->cmd, ocl->latency);   // ocse 5.0
		handle_vc2_cmd(ocl, ocl->cmd, ocl->latency); // adds a vc2 command strucure
		handle_vc2_dcp2_data(ocl->cmd);              // adds data to an existing vc2 command structure
		handle_vc3_cmd(ocl->cmd, ocl->latency);      // adds a vc3 command structure
		handle_vc3_dcp3_data(ocl->cmd);              // adds data to and existing vc3 command structure
		handle_sync(ocl->cmd);
		handle_afu_tlx_write_cmd(ocl->cmd);          // sends a completed write command to client
...
static void _handle_client(struct ocl *ocl, struct client *client)
{
	...
	if (bytes_ready(client->fd, 1, &(client->abort))) {
		if (get_bytes(client->fd, 1, buffer, ocl->timeout,
		  ...
		  info_msg("We got 0x%02x from client on socket 0x%02x", buffer[0], client->fd);
		switch (buffer[0]) {
		case OCSE_DETACH:
