...
{
	printf("lgt:afu_tlx_read_initial_credits and event->afu_tlx_credit_req_valid=0x%x \n", 
...

{
	...
	// at the TLX level??
					...
					*rdata_bad = event->afu_tlx_rdata_bad;
					memcpy(rdata_bus, event->afu_cfg_rdata_bus, 4);
					//return TLX_SUCCESS;
					}
...
int tlx_get_afu_events(struct AFU_EVENT *event)
{
	...
	if (event->rbp != 0) {
		...
		if ( bc == 0 ) {
		         printf("tlx_get_afu_events: bc = 0 after trying to reading data sizes from rbuf\n" );
			return -1;
		}
