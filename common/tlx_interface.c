...
/* For TLX out-bound haX parity buses, generate Odd parity bit for a specified
 * data size set TODO DON"T THINK WE NEED THIS FOR tlx_interface  */
...
static int establish_protocol(struct AFU_EVENT *event)
{
	...
	// Check for broken levels TODO DON"T THINK WE NEED TO DO THIS ANYMORE
	if ((primary == 0) && (secondary == 9908) && (tertiary == 0)) {
		printf("Remote tlx_interface code using broken code level!\n");
	...
	if (primary != event->proto_primary) {
		...
		return TLX_SUCCESS;
         ...
         /* comment out for now TODO DON"T THINK WE NEED THIS ANYMORE
...
int tlx_serv_afu_event(struct AFU_EVENT *event, int port)
{
	...
	event->room = 64;
	event->rbp = 0;
	event->dma0_wr_credits = MAX_DMA0_WR_CREDITS; //TODO not needed?
	event->dma0_rd_credits = MAX_DMA0_RD_CREDITS; //TODO not needed?
        printf("tlx_serv_afu_event: rd_credit count is %d  wr_credit count is %d \n", event->dma0_rd_credits, event->dma0_wr_credits);
...
int tlx_aux1_change(struct AFU_EVENT *event, uint32_t room)
{
	if (event->aux1_change) {
		return TLX_DOUBLE_COMMAND;
	} else {
		event->aux1_change = 1;
		event->room = room;
		return TLX_SUCCESS;
...
int
tlx_job_control(struct AFU_EVENT *event, uint32_t job_code, uint64_t address)
{
	if (event->job_valid) {
		return TLX_DOUBLE_COMMAND;
	} else {
...
int
tlx_mmio_read(struct AFU_EVENT *event,
...
int
tlx_mmio_write(struct AFU_EVENT *event,
	       uint32_t dbl,
...
/* Call this to create a TLX command response */

int
tlx_response(struct AFU_EVENT *event,
	     uint32_t tag,
...
/* Call this to read a buffer TODO this has to change to read CMD DATA bus*/
...
int
tlx_buffer_read(struct AFU_EVENT *event,
...
int
tlx_buffer_write(struct AFU_EVENT *event,
		 uint32_t tag,
		 uint64_t address,
...
*/
int
tlx_dma0_cpl_bus_write(struct AFU_EVENT *event,
		 uint32_t utag,
		 uint32_t cpl_type,
		 uint32_t dsize, 
		 uint32_t cpl_laddr,
...
int
tlx_dma0_sent_utag(struct AFU_EVENT *event,
		 uint32_t utag,
		 uint32_t sent_sts)
{
...
int
tlx_get_mmio_acknowledge(struct AFU_EVENT *event,
			 uint64_t * read_data, uint32_t * read_data_parity)
{
	if (!event->mmio_ack) {
		return TLX_MMIO_ACK_NOT_VALID;
	} else {
		event->mmio_ack = 0;
		event->mmio_valid = 0;
...
int
tlx_get_aux2_change(struct AFU_EVENT *event,
...
int
tlx_get_buffer_read_data(struct AFU_EVENT *event,
			 uint8_t * read_data, uint8_t * read_parity)
{
	if (!event->buffer_rdata_valid) {
		return TLX_BUFFER_READ_DATA_NOT_VALID;
	} else {
...
/* Call after an event is received from the AFU to extract a TLX command
 * if available.
 * Note: fields in AFU_EVENT structre can also be accessed directly */

int
tlx_get_command(struct AFU_EVENT *event,
		uint32_t * command,
 ...
 * tlx_aux1_change, tlx_job_control, tlx_mmio_read, tlx_mmio_write,
 * tlx_response, tlx_buffer_read, tlx_buffer_write */
...
int tlx_signal_afu_model(struct AFU_EVENT *event)
{
	...
	if (event->response_valid != 0) {
	        printf( "lgt: tlx_signal_afu_model: response: tag=0x%02x, tag parity=0x%02x, code=0x%02x \n", 
			event->response_tag, 
	...
	// for now, we always return the default value for pagesize, set by tlxse.parms
		event->tbuf[bp++] = (event->response_r_pgsize & 0x0F);
		event->response_valid = 0;
	}
	if (event->buffer_read != 0) {
	  //printf( "lgt: tlx_signal_afu_model: buffer read: tag: 0x%02x, tag parity: 0x%02x\n", 
	...
	if (event->buffer_write != 0) {
	  // printf( "lgt: tlx_signal_afu_model: buffer write: tag: 0x%02x, tag parity: 0x%02x\n", 
...
/* Call this to send an event to the TLX model */
/* UPDATE: Now static as it's called in tlx_get_tlx_events() */
...
// dma0 request read or write  
		...
		//printf("TLX_SIGNAL_TLX_MODEL: event->dma0_dvalid =1 send to TLX, tbuf[0] is 0x%02x  bp is %2d \n", event->tbuf[0], bp);
		event->dma0_dvalid = 0;
	}
...
//printf("TLX_SIGNAL_TLX_MODEL: event->mmio_ack =1 send to TLX, tbuf[0] is 0x%02x  bp is %2d \n", event->tbuf[0], bp);
	...
	if (event->command_valid) {
		...
		//printf("TLX_SIGNAL_TLX_MODEL: event->command_valid =1 send to TLX, tbuf[0] is 0x%02x  bp is %2d \n", event->tbuf[0], bp);
		event->command_valid = 0;
	}
 ...
 * tlx_get_command
 * tlx_get_buffer_read_data
 * tlx_get_mmio_acknowledge
 * A tlx command can come at any time so that function should always be called
 * but buffer read data and MMIO acknowledges will only come as a result of
 * actions from the TLX simulation so if it is known that there are no
...
//printf("TLX_GET_AFU_EVENT-2 - rbuf[0] is 0x%02x and rbc is %2d \n", event->rbuf[0], rbc);
		...
		// this only gets us a dma rd op w/o data, have to add more to rbc so need to see what dtype  & req_size are)
				...
				//printf("tlx_get_afu_events and we have a dma op \n");
				event->rbp += bc;
		...
		// If this is an Atomic Memory Op, will have one extra char for atomic_op and 16 for data payload
				if ((event->rbuf[1] & 0x07) == DMA_DTYPE_ATOMIC) 
					rbc += 17;
//printf("TLX_GET_AFU_EVENT-3 - rbuf[0] is 0x%02x and rbc is %2d \n", event->rbuf[0], rbc);
			}
	}
	if ((bc =
...
//printf("TLX_GET_TLX_EVENTS event->rbuf[0] is 0x%2x and event->rbuf[1] is 0x%2x \n", event->rbuf[0], event->rbuf[1]);
	if (((event->rbuf[0] & 0x80) == 0x80) && ((event->rbuf[1] & 0xF0) != 0xF0)) {
		event->dma0_completion_valid = 1;
//printf("TLX_GET_TLX_EVENTS setting event->dma0_completion_valid to 1 \n");
		...
		event->dma0_completion_laddr = (( (ex & 0x0f0) <<4) | event->dma0_completion_laddr );
		//event->dma0_completion_laddr = ((event->rbuf[rbc++] << 4) | event->dma0_completion_laddr);
		//printf("event->dma0_completion_laddr = 0x%3x \n", event->dma0_completion_laddr);
		event->dma0_completion_byte_count = (( (ex & 0x0f) <<8) | event->dma0_completion_byte_count );
	...
	if (((event->rbuf[0] & 0x80) == 0x80) && ((event->rbuf[1] & 0xF0) == 0xF0)) {
		event->dma0_sent_utag_valid = 1;
		//printf("TLX_GET_TLX _EVENTS setting event->dma0_sent_utag_valid to 1 \n");
	...
	if (event->rbuf[0] & 0x04) {
		...
		printf( "lgt: tlx_get_tlx_events: respsonse tag=0x%02x, tag parity=0x%02x, respsonse code=0x%02x. respsonse itag=0x%02x, \n", event->response_tag, event->response_tag_parity, event->response_code, event->response_dma0_itag );
	...
	if (event->rbuf[0] & 0x02) {
		...
		event->buffer_read_address = (event->rbuf[rbc++]) & 0x3F;
	        printf( "lgt: tlx_get_tlx_events: buffer read: tag: 0x%02x, tag parity: 0x%02x\n", 
		   	event->buffer_read_tag, 
			event->buffer_read_tag_parity);
	} else {
		event->buffer_read = 0;
	...
	if (event->rbuf[0] & 0x01) {
		...
		for (bc = 0; bc < 2; bc++) {
	        ...
	        printf( "lgt: tlx_get_tlx_events: buffer write: tag: 0x%02x, tag parity: 0x%02x\n", 
		   	event->buffer_write_tag, 
			event->buffer_write_tag_parity);
	} else {
		event->buffer_write = 0;
...
/* Call this on the AFU side to build a command to send to TLX */

int
tlx_afu_command(struct AFU_EVENT *event,
		uint32_t tag,
		uint32_t tag_parity,
		uint32_t code,
...
// instead of sent_utag_sts bc there won't be a sent utag sts for aborts
		if (code == TLX_COMMAND_ITAG_ABRT_RD)
			event->dma0_rd_credits +=1;
		if (code == TLX_COMMAND_ITAG_ABRT_WR)
...
int
tlx_afu_mmio_ack(struct AFU_EVENT *event,
		 uint64_t read_data, uint32_t read_data_parity)
{
	if (event->mmio_ack) {
		return TLX_DOUBLE_COMMAND;
...
int
tlx_afu_read_buffer_data(struct AFU_EVENT *event,
			 uint32_t length,
			 uint8_t * read_data, uint8_t * read_parity)
{
	if (event->buffer_rdata_valid) {
...
int
tlx_afu_aux2_change(struct AFU_EVENT *event,
		    uint32_t running,
		    uint32_t done,
		    uint32_t cack_llcmd,
		    uint64_t job_error,
...
int
tlx_afu_dma0_req(struct AFU_EVENT *event,
		uint32_t utag,
		uint32_t itag,
		uint32_t type,
		uint32_t size,
...

{
	...
	if ((event->dma0_req_type == DMA_DTYPE_RD_REQ) && (event->dma0_rd_credits <= 0))  {
		...
		return TLX_NO_DMA_PORT_CREDITS; }
	...
	// For DMA write or atomic op need to send data - TODO add support for >128 up to 512B writes
		...
		//printf("tlx_afu_dma0_req: rd_credit count is %d  wr_credit count is %d \n", event->dma0_rd_credits, event->dma0_wr_credits);
		event->dma0_dvalid = 1;			
		return TLX_SUCCESS;
