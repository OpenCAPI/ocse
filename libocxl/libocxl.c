...
static int _handle_wake_host_thread(struct ocxl_afu_h *afu)
{
	...
	debug_msg("AFU WAKE HOST THREAD REQUEST");
	//buffer[0] = OCSE_WAKE_HOST_THREAD (already read)
	//buffer[1] = event->cmd_flag
	//buffer[2] = event->obj_handle 
	...
	if (get_bytes_silent(afu->fd, 8, adata, 1000, 0) < 0) {
		warn_msg("Socket failure getting obj_handle");
		_all_idle(afu);
		return -1;
	}
	memcpy(&obj_handle, adata, 8);
	debug_msg("_handle_interrupt: received wake host thread obj_handle  0x%016lx", obj_handle);

	printf("_handle_wake_host_thread: got all the way here but what next?\n");
	return 0;
